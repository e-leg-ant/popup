...
class PopupController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'widget', 'seen'),
				'users'=>array('*'),
			),
    ...
    public function actionWidget($id)
    {
        ...
        $model = Popup::model()->findByPk((int)$id);

        if (!empty($model) && 1 == $model->active) {

            $css = str_replace(array("\r", "\n"), '', $this->renderPartial('widget/css', array(
                'model' => $model,
            ), true, true));

            $html = str_replace(array("\r", "\n"), '', $this->renderPartial('widget/html', array(
                'model' => $model,
            ), true, true));

            echo $this->renderPartial('widget/js', array(
                'model' => $model,
                'html' => $html,
                'css' => $css,
            ));
